...
async function loadHesaplar() {
  ...
  let toplam = 0;
  container.innerHTML = hesaplar
    .map((h) => {
      toplam += h.bakiye;
      return `<div class="rounded-xl p-4 shadow flex flex-col bg-gradient-to-r ${
        h.tur === "Nakit"
          ...
          : "from-red-400 to-orange-400"
            ...
            <div class="flex justify-between items-center">
                ...
                <span class="text-xs">${
                  h.son_kullanim ? "Son kullanım: " + h.son_kullanim : ""
                }</span>
            </div>
            <div class="text-2xl font-bold mt-2">₺${h.bakiye.toLocaleString(
              "tr-TR",
    ...
    .join("");
  document.getElementById("bakiye").innerText = `₺${toplam.toLocaleString(
    "tr-TR",
    { minimumFractionDigits: 2 }
  )}`;
}
    ...
    .map((i) => {
      ...
      return `<div class="flex items-center justify-between border-b pb-1">
            <div class="flex items-center">
                <span class="text-2xl mr-2 ${renk}">${
        i.tur === "gelir" ? "＋" : i.tur === "gider" ? "－" : "⇄"
      }</span>
                <div>
            ...
            <div class="font-bold ${renk}">${isaret}${i.miktar.toLocaleString(
        "tr-TR",
        { minimumFractionDigits: 2 }
      )} TL</div>
        </div>`;
...
function fillAySec() {
  ...
  aySec.innerHTML = aylar
    .map(
      (ad, i) =>
        `<option value="${i + 1}" ${
          i === now.getMonth() ? "selected" : ""
        }>${ad} ${now.getFullYear()}</option>`
    )
    .join("");
...
async function loadAylikOzet(ay = null) {
  ...
  try {
    ...
    if (
      ...
      document.getElementById("aylikOzet").innerHTML = data.error
        ? data.error
        : "Veri yok";
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (gelirEkleBtn) {
    gelirEkleBtn.onclick = async function () {
      ...
      const hesapSecOptions = hesaplar
        .map((h) => `<option value="${h.id}">${h.ad}</option>`)
        .join("");
      modalAc(`
  ...
  if (raporBtn) {
    raporBtn.onclick = async function () {
      ...
      const res = await fetch(
        `/api/rapor?yil=${now.getFullYear()}&ay=${now.getMonth() + 1}`
      );
      ...
      modalAc(`
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-semibold mb-4">Aylık Özet</h3>
                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                        ...
                        <div class="flex justify-between items-center border-t pt-2">
                            <div class="text-sm font-medium text-gray-700">Net Durum</div>
                            <div class="text-lg font-bold ${raporData.netDurum >= 0 ? 'text-green-600' : 'text-red-600'}">
                                ₺${raporData.netDurum.toLocaleString('tr-TR', {minimumFractionDigits:2})}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- En Yüksek Giderler -->
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-semibold mb-2">En Yüksek Gider Kategorileri</h3>
                    <div class="space-y-2">
                        ${raporData.enYuksekGiderler
                          .map(
                            (k) => `
                            <div class="flex justify-between items-center">
                                <span class="text-sm">${k.kategori}</span>
                                <span class="font-semibold text-red-600">₺${k.toplam.toLocaleString(
                                  "tr-TR",
                                  { minimumFractionDigits: 2 }
                                )}</span>
                            </div>
                ...
                <!-- Hesap Bazlı İşlemler -->
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-semibold mb-2">Hesap Bazlı İşlemler</h3>
                    <div class="space-y-3">
                        ${raporData.hesapIslemleri
                          .map(
                            (h) => `
                            <div class="border-b pb-2">
                                <div class="font-medium mb-1">${h.hesap}</div>
                                <div class="grid grid-cols-3 gap-2 text-sm">
                                    <div>
                                        <span class="text-gray-600">Gelir:</span>
                                        <span class="text-green-600">₺${h.gelir.toLocaleString(
                                          "tr-TR",
                                          { minimumFractionDigits: 2 }
                                        )}</span>
                                    </div>
                                    <div>
                                        ...
                                        <span class="text-red-600">₺${h.gider.toLocaleString(
                                          "tr-TR",
                                          { minimumFractionDigits: 2 }
                                        )}</span>
                                    </div>
                ...
                <!-- Günlük İşlemler -->
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h3 class="font-semibold mb-2">Günlük İşlemler</h3>
                    <div class="space-y-2">
                        ${raporData.gunlukIslemler
                          .map(
                            (i) => `
                            <div class="flex justify-between items-center">
